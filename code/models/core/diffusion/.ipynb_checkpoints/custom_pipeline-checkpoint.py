...
def generate_ip_adapter_embeds(
    ...
    **kwargs,
    ...
    negative_add_time_ids = (
        self._get_add_time_ids(negative_original_size, negative_crops_coords_top_left, negative_target_size, dtype=prompt_embeds.dtype, text_encoder_projection_dim=text_encoder_projection_dim)
            if negative_original_size is not None and negative_target_size is not None else 
        add_time_ids
    )
...
class Generator4Embeds:
    def __init__(self, num_inference_steps=1, device='cuda') -> None:
        ...
        pipe = DiffusionPipeline.from_pretrained("/workspace/sdxl-turbo", torch_dtype=torch.float16, variant="fp16")
        ...
        # load ip adapter
        pipe.load_ip_adapter("/workspace/IP-Adapter", subfolder="sdxl_models", weight_name="ip-adapter_sdxl_vit-h.safetensors", torch_dtype=torch.float16)
    ...
    def generate(self, image_embeds, text_prompt='', generator=None):
        ...
        image = pipe.generate_ip_adapter_embeds(prompt=text_prompt, ip_adapter_embeds=image_embeds, num_inference_steps=self.num_inference_steps, guidance_scale=0.0, generator=generator).images[0]
        return image
...
if __name__ == "__main__":
    ...
    image_encoder, _, feature_extractor = open_clip.create_model_and_transforms('ViT-H-14', pretrained='laion2b_s32b_b79k', precision='fp16', device='cuda')
